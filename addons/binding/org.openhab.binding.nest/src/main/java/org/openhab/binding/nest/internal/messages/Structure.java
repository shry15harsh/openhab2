...
public class Structure extends AbstractMessagePart implements DataModelElement {
    ...
    public void sync(DataModel dataModel) {
        // Build named-based maps from ID-bpl@123
        ...
        if (this.thermostat_id_list != null) {
            for (String id : this.thermostat_id_list) {
                Thermostat th = null;
                if (dataModel.getDevices().getThermostats_by_id() != null) {
                    th = dataModel.getDevices().getThermostats_by_id().get(id);
        ...
        if (this.smoke_co_alarm_id_list != null) {
            for (String id : this.smoke_co_alarm_id_list) {
                SmokeCOAlarm sm = null;
                if (dataModel.getDevices().getSmoke_co_alarms_by_id() != null) {
                    sm = dataModel.getDevices().getSmoke_co_alarms_by_id().get(id);
                }
                if (sm != null) {
        ...
        if (this.camera_id_list != null) {
            for (String id : this.camera_id_list) {
                Camera cam = null;
                if (dataModel.getDevices().getCameras_by_id() != null) {
                    cam = dataModel.getDevices().getCameras_by_id().get(id);
                }
                if (cam != null) {
